...
function hint(skill, p) {
  return [
    ...
    ``,
    `**막혔다면 이렇게 시작하세요**`,
    `1. 문제에서 *주어진 값*과 *구해야 할 값*을 따로 적어 보세요.`,
    `2. 둘을 이어 줄 공식이나 성질을 한 줄로 떠올려 보세요.`,
    `3. 계산을 시작하기 전에 부호( + / − )와 조건을 먼저 정리하세요.`,
    ``,
    `**확인 포인트**`,
    `- 단위·괄호·분모가 빠지지 않았는지 점검하세요.`,
    `- 답이 자연수/분수/소수 중 어떤 형태여야 하는지 미리 예상해 보세요.`,
  ].join("\n");
}
function next(skill, p) {
  return [
    ...
    `2. 사용할 공식이나 성질을 식 옆에 메모해 둡니다.`,
    `3. 값을 대입하거나 항을 정리해서 *첫 번째 중간식*을 만듭니다.`,
    `4. 바뀌는 부분만 한 줄씩 고쳐 가며 답까지 이어 갑니다.`,
    `5. 마지막에 구한 값을 원래 식에 넣어 맞는지 검산합니다.`,
    ``,
